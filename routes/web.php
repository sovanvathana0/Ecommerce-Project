...
// Cart section (changed 'user' middleware to 'auth')
...
Route::get('/wishlist/{slug}', [WishlistController::class, 'wishlist'])->name('add-to-wishlist')->middleware('auth');
...
// User section start (changed 'user' middleware to 'auth')
...
Route::get('/test-mail', function () {
    ...
    if (!$order) return 'No order in DB.';
    \Illuminate\Support\Facades\Mail::to(auth()->user()->email)
        ->send(new \App\Mail\OrderConfirmationMail($order));
    return 'Test mail sent to ' . auth()->user()->email;
});